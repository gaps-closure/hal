...
void *shm_dev_ON_r(device *d) {
  log_trace("Turn SHM-R %s ON (fd r=%d w=%d r=%s w=%s) if not already on", d->id, d->read_fd, d->write_fd, d->path_r, d->path_w);
  return (shm_dev_ON(d, &(d->read_fd), d->path_r, d->addr_off_r));
}

/* Open SHM WRITE device */
void *shm_dev_ON_w(device *d) {
  log_trace("Turn SHM-W %s ON fd=(r=%d w=%d) path=(r=%s w=%s) if not already on", d->id, d->read_fd, d->write_fd, d->path_r, d->path_w);
...
void shm_dev_OFF(device *d) {
  ...
  d->read_fd = -1;
  log_trace("Turned SHM %s OFF fd=(r=%d w=%d) path=(r=%s w=%s)", d->id, d->read_fd, d->write_fd, d->path_r, d->path_w);
}
