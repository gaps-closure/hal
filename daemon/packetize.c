...
/* Write into internal PDU (out) an input packet (in) or directly from idev */
int pdu_from_packet(pdu *out, uint8_t *in, int len_in, device *idev) {
  ...
  else if (strcmp(idev->model, "sdh_sm_v1")    == 0) pdu_len = pdu_from_sdh_sm_v1 (out, idev);  /* copy from SHM */
  else {log_fatal("%s: unknown interface model: %s", __func__, idev->model); exit(EXIT_FAILURE);}
  return (pdu_len);
}

/* Write internal PDU (in) into packet (out) or directiy to device (odev) */
void pdu_into_packet(uint8_t *out, pdu *in, int *pkt_len, selector *osel, device *odev) {
  ...
  else {log_fatal("%s unknown interface model %s", __func__, dev_model); exit(EXIT_FAILURE);}  /* copy to SHM */
}
