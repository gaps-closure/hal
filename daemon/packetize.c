...
int pdu_from_packet(pdu *out, uint8_t *in, int len_in, device *idev) {
  ...
  else if (strcmp(idev->model, "sdh_socat_v1") == 0) pdu_len = pdu_from_sdh_sc_v1 (out, in, len_in);
...
void pdu_into_packet(uint8_t *out, pdu *in, int *pkt_len, selector *osel, const char *dev_model) {
  ...
  else if (strcmp(dev_model, "sdh_socat_v1") == 0)  *pkt_len = pdu_into_sdh_sc_v1 (out, in, &(osel->tag));
