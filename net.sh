...
while true; do
  ...
  case $APP_INDEX in
    1)
      ...
      echo "$DATA1" | stdbuf -oL xxd -r -p  > $FIFO_NAME
      ;;
    2)
      echo "Sending to netcat: $DATA2"
      echo "$DATA2" | stdbuf -oL xxd -r -p  > $FIFO_NAME
      ;;
    11)
      echo "Sending to netcat: $DATA1"
      echo "$DATA1" | stdbuf -oL xxd -r -p  > $FIFO2
      ;;
    12)
      echo "Sending to netcat: $DATA2"
      echo "$DATA2" | stdbuf -oL xxd -r -p  > $FIFO2
      ;;
    *)
