...
#  print (results)
  ...
  with open(fn_out, 'w') as result_file:
    ...
    wr.writerow(['RX Sequence Number', 'TX Sequence Number', 'time received (secs)', 'time sent (secs)', args.req_rate])
    wr.writerows(results)
    

# Get matching input file and create a unique name for output file
def get_files(fnA):
...
if __name__=='__main__':
  ...
  parser.add_argument('-A', '--fname_in_A', help='Node A hostname: default uses input file prefix', type=str, default='')
  parser.add_argument('-b', '--fname_pf_B', help='Node B halperf results input file prefix: default = orange', type=str, default='orange')
  parser.add_argument('-B', '--fname_in_B', help='Node B hostname: default uses input file prefix', type=str, default='')
  ...
  parser.add_argument('-r', '--req_rate',   help='Requested rate: default = 0', type=int, default=0)
