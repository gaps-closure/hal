...
latency, throughput
hearbeat and mission plan
with and without redaction
update mission plan XD

active MQ produces
consumer measures 
# EOP Mission APP:
# EGRESS = orange (pitcher) to green (catcher
DATA_TYP_HB_EGRESS         = 13     # Cases: 1, 2
DATA_TYP_REQ_EOIR_EGRESS   = 15     # Cases: 1 (size 48)
DATA_TYP_PLAN_EGRESS       = 17     # Cases: 1, 2
# INGRESS = green to orange
DATA_TYP_HB_INGRES         = 113    # Cases: 1, 2
DATA_TYP_MOVE_INGRES       = 115    # Cases: 1

DATA_TYP_PLAN_UPD_EGRESS   = 19     # Cases: 2

DATA_TYP_REC_ISRM_INGRES   = 115    # Cases: 2
DATA_TYP_REC_EOIR_INGRESS  = 117    # Cases: 2, 3
...
class MA_Hb(Structure):
    _fields_ = [("data", c_ubyte * 112)]

class MA_mission_plan(Structure):
    _fields_ = [("data", c_ubyte * 736)]

class isrm_detectStructure):
    _fields_ = [("data", c_ubyte * 736)]
...
if __name__ == '__main__':
    ...
    data = b'\x01\x23\x34\x45\x56\x78\x89\xab\xcd\xef\x01\x23\x34\x45\x56\x78\x89\xab\xcd\xef'
    print(" ".join(hex(n) for n in data))
    adu = MA_Hb()
    adu.data=(c_ubyte * sizeof(adu.data))(*data)
#    print ('data = ', bytes(adu.data), 'size =', sizeof(adu.data))
    print(" ".join(hex(n) for n in adu.data))
