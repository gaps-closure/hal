...
while true; do
  ...
  case $APP_INDEX in
    1)
      ...
      echo "$DATA_BE1_01" | stdbuf -oL xxd -r -p  > $FIF0_PORT1
      ;;
    2)
    ...
    13)
      ...
      ;;
    30)
      echo "Sending to $IF_NET3 IP=$IP_ADDR send port=$IP_PORT4 prot=TCP: $DATA_BW1_04"
      echo "$DATA_BW1_04" | stdbuf -oL xxd -r -p  > $FIF0_PORT4
      ;;
    *)
