...
/**********************************************************************/
int isDroppable(){
  const char* s = getenv("LOSS");
  int loss = (s==NULL) ? 0 : atoi(s);
  static int init = 0;
  if(!init){
    srand(1000);
    init = 1;
  ...
  int random = rand() % 100;
  if(random < loss){
      //fprintf(stderr,"Dropped %s %d %d\n",s, loss, random);
      return 1;
  }
    //fprintf(stderr,"Not Dropped %d\n",random);
  return 0;
}

void delay(){
  const char* s = getenv("DELAY");
  int delay = (s==NULL) ? 0 : atoi(s);
  if(delay == 0) return;
  double random = (rand() % delay)/1000.0;
  //fprintf(stderr,"Delayed by %f\n", random);
  sleep(random); 
}
...
void xdc_asyn_send(void *socket, void *adu, gaps_tag *tag)
{
  ...
  if (bytes <= 0)
    log_error("RCV ERROR on ZMQ socket %d: size=%d err=%s", socket, bytes, zmq_strerror(errno));
}
...
int xdc_recv(void *socket, void *adu, gaps_tag *tag)
{
  ...
  {
    if (errno == EAGAIN)
      log_trace("Timeout on socket (%p): rv=%d error = %s", socket, size, zmq_strerror(errno));
    else
      log_error("RCV ERROR on ZMQ socket (%p): rv=%d error = %s", socket, size, zmq_strerror(errno));
  }
  else
  {
